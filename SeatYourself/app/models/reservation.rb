class Reservation < ActiveRecord::Base
  ...
  def valid_date_range
    if !date.blank? && date > Date.today + 30.days
      errors.add(:date, "for bookings can only be made 30 days in advance.")
    end
  end

  def future_date_only
    if !date.blank? && date <= Date.today
      errors.add(:date, "for bookings can only be made for a later date.")
    end
  end
  ...
  def availability
    ...
    if v_party_size != 0 && ((reserved_seats + v_party_size) > available_seats)
      errors.add(:party_size, "too large, there are no available bookings.")
    end
  end

  def hours_of_operation
    if !time.blank? && !time.between?(restaurant.opening, restaurant.closing)
      errors.add(:time, "for reservation not available! Boo.")
    end
  end
